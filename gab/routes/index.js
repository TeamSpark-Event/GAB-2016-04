...
router.get('/', function(req, res, next) {
    var model = {
      ...
      slots: [
          {title: 'Сборка, развертывание и управление релизами в облаке.', desc: 'Еще с давних времен было популярно разворачивать свое приложение при помощи разного рода графических инстументов в режиме publish -> next -> next -> finish. Этот подход использовали многие, так как он был простым и понятным. Но у него была и есть одна большая беда - его практически невозможно автоматизаровать. Других, более продвинутых инструментов, мноие боялись, так как думали что они сложны в управлении. Во время этого доклада будет развеян этот миф и мы покажем, как меньше чем за час можно настроить сборку приложения в облаке, а также его развертывание на разные окружения используя при этом простые и понятные инстументы.'},
          {title: 'Azure Service Fabric и архитектура микросервисов.', desc: 'Традиционный подход к проектированию монолитных приложений по своему хорош, но у него все же есть недостатки. Монолит очень тяжело масштабировать и не всегда просто понять принцип его работы для новых людей, которые приходят в команду. Альтернативой монолиту есть подход с гордым и романтичным названием - архитектура микросервисов. В рамках доклада мы немного поговорим о самой архитектуре в целом, а также затроним новый фреймворк Service Fabric. Этот фреймворк является открытым, его можно хостить не только в Azure, но именно тут вы получите больше преимуществ от его использования. Service Fabric позволяет мне, как разработчику, сфокусироваться на создании нового кода, а задачи развертывание, обеспечение высокой доступности и взаимодействия между собой моих микросервисов уже решены и они не отвлекают меня от главного - создания нового продукта.'},
          {title: 'Как разместить надежный и масштабируемый сайт со своим доменным именем в облаке не превышаю бюджет в $1?', desc: 'Как вы возможно знаете, в облаке Microsoft Azure есть бесплатный хостинг план для веб-сайтов, но он имеет ряд ограничений, таких как 60 минут процессорного времени, 165 МБ траффика в день и отсутствие возможности назначить собственное доменное имя. Есть ли способ обойти эти ограничения? Да, он есть!'},
          {title: 'Azure Active Directory - взгляд со стороны безопасности.', desc: 'Доклад ориентирован на ознакомление с возможностями Azure Active Directory в области управлении идентификацией в облачных приложениях. Будут рассмотрены сценарии только облачного использования, а так же в интеграции с наземным ADDS.'},
